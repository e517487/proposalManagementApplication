...
type EntityResponseType = HttpResponse<IRecord10AanvraagGegevensAlgemeenMySuffix>;
type EntityArrayResponseType = HttpResponse<IRecord10AanvraagGegevensAlgemeenMySuffix[]>;

@Injectable({ providedIn: 'root' })
export class Record10AanvraagGegevensAlgemeenMySuffixService {
    private resourceUrl = SERVER_API_URL + 'api/record-10-aanvraag-gegevens-algemeens';

    constructor(private http: HttpClient) {}

    create(record10AanvraagGegevensAlgemeen: IRecord10AanvraagGegevensAlgemeenMySuffix): Observable<EntityResponseType> {
        const copy = this.convertDateFromClient(record10AanvraagGegevensAlgemeen);
        return this.http
            .post<IRecord10AanvraagGegevensAlgemeenMySuffix>(this.resourceUrl, copy, { observe: 'response' })
            .map((res: EntityResponseType) => this.convertDateFromServer(res));
    }

    update(record10AanvraagGegevensAlgemeen: IRecord10AanvraagGegevensAlgemeenMySuffix): Observable<EntityResponseType> {
        const copy = this.convertDateFromClient(record10AanvraagGegevensAlgemeen);
        return this.http
            .put<IRecord10AanvraagGegevensAlgemeenMySuffix>(this.resourceUrl, copy, { observe: 'response' })
            .map((res: EntityResponseType) => this.convertDateFromServer(res));
    }

    find(id: number): Observable<EntityResponseType> {
        return this.http
            .get<IRecord10AanvraagGegevensAlgemeenMySuffix>(`${this.resourceUrl}/${id}`, { observe: 'response' })
            .map((res: EntityResponseType) => this.convertDateFromServer(res));
    }
    ...
    query(req?: any): Observable<EntityArrayResponseType> {
        ...
        return this.http
            .get<IRecord10AanvraagGegevensAlgemeenMySuffix[]>(this.resourceUrl, { params: options, observe: 'response' })
    ...
    ): IRecord10AanvraagGegevensAlgemeenMySuffix {
        const copy: IRecord10AanvraagGegevensAlgemeenMySuffix = Object.assign({}, record10AanvraagGegevensAlgemeen, {
            aanvangDatum:
    ...
    private convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
        res.body.forEach((record10AanvraagGegevensAlgemeen: IRecord10AanvraagGegevensAlgemeenMySuffix) => {
